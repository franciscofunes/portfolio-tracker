...
export const TopBar = () => {
  ...
  return (
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mb-6 w-full mt-8">
      ...
      <div className="font-semibold border rounded px-4 py-2 flex flex-col sm:flex-row items-start sm:items-center sm:space-x-3 w-full sm:w-auto">
        <PortfolioSelector
          portfolios={portfolios.map((p) => ({ id: p.id, name: p.title }))}
          selectedId={selectedPortfolioId || portfolios[0]?.id || ""}
          onChange={setSelectedPortfolioId}
        />
        <span className="font-mono whitespace-nowrap">${portfolioValue.toFixed(2)}</span>
      </div>
    </div>
  );
};